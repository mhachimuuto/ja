...
`ORA-15041: diskgroup "ARCH" space exhausted` と `ORA-00257`！この2つのエラーコードが揃ったことで、原因は明確になりました。**アーカイブ・ログの格納に使用される `+ARCH` ディスクグループが100%フルになっていたのです。これによりアーカイバ・プロセスが停止し、データベースはデータ整合性を保護するために自己防衛的にハングアップし、すべての新規トランザクション要求を拒否していました。**
